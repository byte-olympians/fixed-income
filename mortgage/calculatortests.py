...
class TestCalculator(unittest.TestCase):
    ...
    def test_calculate_monthly_payment(self):
        monthly_payment = round(MortgageFormulas.calculate_monthly_payment(self.principal,
                                                                     self.rate,
                                                                     self.term),
                                2)
    ...
    def test_convert_rate_to_monthly(self):
        self.assertEqual(0.0054, round(MortgageFormulas.convert_rate_to_monthly(self.rate), 4),
                         "The monthly rate for 6.5 should be 0.0054")

    def test_convert_term_to_monthly(self):
        self.assertEqual(360, MortgageFormulas.convert_term_to_monthly(self.term),
                         "The monthly term for a 30 year term is 360")

    def test_calculate_loan_balance_start(self):
        months_elapsed = 0

        loan_balance = round(MortgageFormulas.calculate_loan_balance(self.principal,
                                                               self.rate,
                                                               self.term,
                                                               months_elapsed), 0)
        self.assertEqual(
    ...
    def test_calculate_monthly_interest(self):
        monthly_interest = MortgageFormulas.calculate_monthly_interest(
    ...
    def test_calculate_mortgage_metrics(self):
        ...
        self.assertEqual(
            mortgage_metrics.totals['payments'], 1000, "The payments must total 1000")
        self.assertEqual(
            mortgage_metrics.totals['interest'], 750, "The interest must total 250")
        self.assertEqual(mortgage_metrics.totals[
                          'principal'], 250, "The total principal paid back must be 750")

        self.assertEqual(mortgage_metrics.metrics[
    ...
    def test_calculate_schedule(self):
        schedule = MortgageCalculator.calculate_schedule(
        ...
        # Check the totals
        self.assertEqual(round(schedule.mortgage_metrics.totals[
        ...
        self.assertEqual(round(schedule.mortgage_metrics.metrics[
        ...
        self.assertEqual(last_payment.cumulative_principal, round(schedule.mortgage_metrics.totals[
                         'principal'], 2), "The cumulative principal for the last monthly payment should equal the total principal repaid")
        self.assertEqual(last_payment.cumulative_interest, round(schedule.mortgage_metrics.totals[
                         'interest'], 2), "The cumulative interest for the last monthly payment should equal the total interest paid")
        self.assertEqual(last_payment.cumulative_payment, round(schedule.mortgage_metrics.totals[
